    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if len(stack)>0:
            ...
            if 'ctag' in token and FeatureExtractor._check_informative(token['ctag'], True):
                result.append('STK_0_CPOSTAG_' +  nltk.tag.mapping.map_tag("en-ptb", "universal", token['ctag']))
        ...
        if buffer:
            ...
            if 'ctag' in token and FeatureExtractor._check_informative(token['ctag'], True):
                result.append('BUF_0_CPOSTAG_' +  nltk.tag.mapping.map_tag("en-ptb", "universal", token['ctag']))
